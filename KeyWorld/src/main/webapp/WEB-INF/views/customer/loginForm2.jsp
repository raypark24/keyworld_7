...
<html>
	<head>
		...
		<script>
			$(function(){
				var sign = '<div class="joinForm"><h2>Sign up</h2><form action="join" method="POST"><table><tr><th>ID</th>'
						+'<td><input type="text" id="userid" name="userid" placeholder="3~10자리 아이디" style = "color: rgb(251,207,53);"/><div id = "result"></div></td></tr><tr>'
						+ '<tr><th>Id Check</th><td><p id="idCheckConfirm" style = "color: rgb(251,207,53);"></p></td></tr>'
						+ '<th>Password</th>'
						+ '<td><input type="password" id="password" name="password" placeholder="3~10자리 비밀번호" style = "color: rgb(251,207,53);" /></td>	</tr>'
					 	...
					 	+ '<tr><th>Confirm</th><td><p id="passwordCheckConfirm" style = "color: rgb(251,207,53);"></p></td></tr>'
						...
						+ '<tr><td colspan= "2"><input type="submit" id="send_btn" value="Sign up" style = "background-color: rgb(251,207,53);width:370px; color: white"/>'
						+ '</td></tr><tr><td><a href="${pageContext.request.contextPath}/" >Back</a></td></tr></table></form></div>';
				...
				$("#submit").on("click", function(){
					...
					$("#password2").on('keyup', passwordCheck);
				})
				...
				function idCheck() {
					...
					if(userid.length < 3 || userid.length > 10) {
						...
						if(userid.charAt(0)< 'a'|| userid.charAt(0)>'z') {
							idConfirm = '아이디는 3~10자리, 영문소문자로 시작해야 함.';
							$("#idCheckConfirm").html(idConfirm);
							return;
						}
						...
						, success : function(resp) {
							...
							} else if (resp == "사용 가능한 아이디") {
								$("#send_btn").prop('disabled', false)
							}
			
							idConfirm = resp;
							$("#idCheckConfirm").text(idConfirm);
						}
					});
				};
